...
export default async function handler(req, res) {
  ...
  if (!html && !url) {
    return res
      .status(400)
      .json({ error: 'Missing required input: provide either `html` or `url`.' });
  }

  let browser;
  try {
    const executablePath = await getExecutablePath();
    const isHeadless = chromium.headless !== undefined ? chromium.headless : 'new';
    ...
    const timeout = Math.min(
      Math.max(parseInt(process.env.HTML_TO_PDF_TIMEOUT_MS || '60000', 10) || 60000, 10000),
      180000
    );
