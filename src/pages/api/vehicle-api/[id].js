export default function handler(req, res) {
  ...
  // Convert id to a number to match the `vehicleListings` data structure
  const vehicle = vehicleListings.find((listing) => listing.id === Number(id));

  if (vehicle) {
    res.status(200).json(vehicle);
  } else {
    res.status(404).json({ error: "Vehicle listing not found" });
  }
}
